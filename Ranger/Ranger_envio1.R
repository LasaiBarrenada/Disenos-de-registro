...
library(ggplot2)
library(data.table)
regresoresRF1 <- readRDS("Datos/regresoresRF_envio1.rds")
...
ggplot(data = df_resulados) +
  ...
  geom_vline(xintercept = num_trees_range[which.min(oob_errors)],
             ...
             linetype = "dashed") +
  labs(
    title = "Evolucion del error vs número Árboles",
    x     = "nÃºmero de Áboles",
    y     = "RMSE",
    color = ""
  ) +
...
dev.off()
pdf(file = "Imagenes/PequeñasScatter.pdf",width = 4.72441,height = 2.75591)
...
indices <- readRDS("Datos/Indice_envio1.rds")
indices$general
