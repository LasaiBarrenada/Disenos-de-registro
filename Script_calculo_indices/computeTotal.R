computeTotal <- function(data, sample, varNames, cellNames, samplingWNames = NULL){
  ...
  idVar <- names(sample)
  idVar_repo <- unname(UnitToIDDDNames(idVar, getDD(data)))
  sample <- copy(sample)
  setnames(sample, idVar_repo)
  data.StQ <- data[sample, on = idVar_repo]
