...
regresores3 <- readRDS("Datos/regresoresenvio3BIEN.rds")
...
regresores3 <- regresoresRF[,(dropvariables):=NULL]
regresores3 <- regresoresRF[,(drop2):=NULL]
regresores3 <- regresores3[!is.na(cn01.fde)] #243 salen
regresores3$cn01a.anterior <- as.factor(regresores3$cn01a.anterior)
regresores3$cn01e.anterior <- as.factor(regresores3$cn01e.anterior)
regresores3$cn01v.anterior <- as.factor(regresores3$cn01v.anterior)
missingsRF <- aggr(regresores3,  col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, labels=names(data), cex.axis=.7, gap=3, ylab=c("Histogram of missing data","Pattern"))
...
library(ggplot2)
library(data.table)
regresoresRF3 <- regresores3

regresoresRF3[imputar == 'Cero']$CN01 = 0
data.train <- regresoresRF3[!is.na(regresoresRF3$CN01) & imputar != "Si"]
data.train$imputado <- "No"

data.control <- regresoresRF3[is.na(regresoresRF3$CN01) | imputar == "Si"]
...
pdf(file = "Imagenes/Envio3/TopVariables.pdf")
list_features %>%
  ...
  theme_bw()
dev.off()
pdf(file = "Imagenes/Envio3/BotVariables.pdf")
list_features %>%
  dplyr::arrange(desc(value)) %>%
  dplyr::top_n(-10) %>%
  ggplot(aes(reorder(feature, -value), value)) +
...
# ValidaciÃƒÂ³n empleando el Out-of-Bag error (root mean squared error)
# ==============================================================================
...
pdf(file = "Imagenes/Envio3/ErrorVSnumtree.pdf",width = 4.72441,height = 2.75591)
ggplot(data = df_resulados) +
  ...
  geom_vline(xintercept = 141,
             ...
             linetype = "dashed") +
  labs(
    title = "Evolucion del error vs numero arboles",
    x     = "numero de arboles",
    y     = "RMSE",
    color = ""
  ) +
...
paste("Valor optimo de num.trees:",141)
...
pdf(file = "Imagenes/Envio3/OOBRMSEvsMTRY.pdf")
...
ggplot(data = grid, aes(x = mtry, y = OOB_RMSE, colour = node.size))+
  ...
  ggtitle(label = "Error OOB vs Mtry by Node.size ",
          subtitle = paste(141," Arboles"))

dev.off()
pdf(file = "Imagenes/Envio3/RMSETrainvsMTRY.pdf")
...
pdf(file = "Imagenes/Envio3/R2vsMTRY.pdf")
...
pdf(file = "Imagenes/Envio3/GrandesScatter.pdf")
...
dev.off()
pdf(file = "Imagenes/Envio3/PequeñasScatter.pdf")
ggplot(data = scatterplotPequeñas , aes(x = Real,y = Prediccion)) +
  ...
  theme_bw()+
  ggtitle("Real vs Predicciones",
          "Unidades pequeÃ±as  CN < 1e7")
dev.off()
pdf(file = "Imagenes/Envio3/TotalScatterUmbral.pdf")
...
theme_bw()
pdf(file = "Imagenes/Envio3/ErrorImputaciones.pdf")
...
pdf(file = "Imagenes/Envio3/ImputacioRFvsINE.pdf")
...
dev.off()
pdf(file = "Imagenes/Envio3/ImputacioRFvsINEUmbral.pdf")
...
saveRDS(dataindices, "Datos/dataindices_envio3.rds")
indices <- readRDS("Datos/Indice_envio3.rds")
...
pdf(file = "Imagenes/Envio3/IndiceMIG3.pdf")
...
dev.off()
pdf(file = "Imagenes/Envio3/IndiceCCAA3.pdf")
...
pdf(file = "Imagenes/Envio3/IndiceGeneral3.pdf")
color = indices$general$index - general.indice$INDICE0
color <- ifelse(color >0, "blue3","red3")
ggplot(data = general.indice, aes(x = "11/3", y = INDICE0))+
  geom_segment( aes(x="11/3", xend="11/3", y=indices$general$index , yend=general.indice$INDICE0),colour = color,linetype = "longdash", show.legend =  F,lineend = "round")+
  geom_point(aes(colour = "INE"))+
  geom_point(data = indices$general, aes(x = "11/3", y = index, colour = "RF" ))+
  theme_bw()+
  ggtitle("Indice General",
          "Envio 3")
dev.off()

pdf(file = "Imagenes/Envio3/IndiceSeccion3.pdf")
